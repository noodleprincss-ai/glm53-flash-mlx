...
from mlx_vlm.models.deepseek_v32 import language as _dsv32
from mlx_vlm.models.deepseek_v32.language import MoEGate
from mlx_vlm.models.switch_layers import SwitchGLU
from mlx_vlm.models.deepseek_v32.language import Model as DSV32Model
...
class ClampedSwiGLU(nn.Module):
    """`swiglu_limit`: the reference clamps gate from above and up on both sides before silu(gate)*up.
    Applied to routed experts, the shared expert and the dense layers alike (Glm5NextTextMLP and
    Glm5NextTextExperts._apply_gate); the upstream mlx-vlm port inherits DeepSeek's unclamped MLPs."""
    ...
    def __call__(self, up: mx.array, gate: mx.array) -> mx.array:
        # SwitchGLU calls activation(x_up, x_gate) -- up first.
...
class Glm5NextMoEGate(MoEGate):
    """Router logits in float32 (`moe_router_dtype: float32`); the base class matmuls in bf16."""

    def __call__(self, x):
        logits = x.astype(mx.float32) @ self.weight.astype(mx.float32).T
        return _dsv32.group_expert_select(
            logits, self.e_score_correction_bias, self.top_k, self.n_group,
            self.topk_group, self.routed_scaling_factor, self.norm_topk_prob,
        )
